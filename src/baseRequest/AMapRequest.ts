...
import AMapConfiguration, { QueryClass } from '../configuration/AMapConfiguration';
...
export default class AMapRequest extends BaseRequest {
    ...
    async keywordsQuery(kds: string, config?: IConfig) { 
        ...
        const result = await this.get(QueryClass.SEARCH);
        return new AMapParser(result);
    }

    async busLinesQuery(kds:string, config?:IConfig) {
        const proxy = this.config as AMapConfiguration;
        const options = proxy.options;
        ...
        const result = await this.get(QueryClass.BUSLINE);
        return new AMapParser(result);
    }
